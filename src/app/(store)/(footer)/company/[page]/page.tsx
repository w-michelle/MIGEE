...
export async function generateStaticParams() {
  const PAGES_QUERY = `*[_type == "footerSection" && slug.current == "company"][0]{
                title,
              pages[]->{
            slug,
            }
...
async function CompanyPage({ params }: { params: Promise<{ page: string }> }) {
  const { page } = await params;

  const SECTION_QUERY = `*[_type == "footerSection" && slug.current == "company"][0]{
            title,
            pages[]->{
            title,
            slug,
